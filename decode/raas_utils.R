...
codonFrequencies <- function(codontable, sort=TRUE, unlist=TRUE) {


    codt <- apply(codontable, 2, sum) # total count
    ...
    codl <- lapply(codl, function(x) x/sum(x)) # codon frequency

    if ( unlist ) {
        codl <- unlist(codl)
        names(codl) <- sub("\\.","-",names(codl))
    }
...
## plot sequence difference logos, adapted
## from the DiffLogo package to allow for customized y-axis limits
