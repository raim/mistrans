...
## protein half-lives
math18.file <- file.path(mam.path,"originalData","41467_2018_3106_MOESM5_ESM.xlsx")
...
plotdev(file.path(fig.path,paste0("protein_halflives_raas_unique")),
        type="png", res=300, width=3.5,height=3.5)
idx <- match(pnms[rownames(pbstat)], names(hlv))
par(mai=c(.5,.5,.1,.1), mgp=c(1.2,.3,0), tcl=-.25)
plotCor(pbstat$median, log(hlv[idx]),
        xlab=expression(log[10]*bar(RAAS)["protein,unique"]),
        ylab=expression(ln(bar(t[1/2])/h)))
dev.off()
plotdev(file.path(fig.path,paste0("protein_halflives_raas_all")),
        type="png", res=300, width=3.5,height=3.5)
idx <- match(pnms[rownames(ptstat)], names(hlv))
par(mai=c(.5,.5,.1,.1), mgp=c(1.2,.3,0), tcl=-.25)
plotCor(ptstat$median, log(hlv[idx]), signif = 1,
        xlab=expression(log[10]*bar(RAAS)["protein,all"]),
        ylab=expression(ln(bar(t[1/2])/h)))
dev.off()
...
for ( ctype in c("Bcells", "NK", "hepatocytes", "monocytes", "neurons") ) {
    ...
    plotdev(file.path(fig.path,paste0("protein_halflives_raas_all_",ctype)),
            type="png", res=300, width=3.5,height=3.5)
    par(mai=c(.5,.5,.1,.1), mgp=c(1.2,.3,0), tcl=-.25)
    idx <- match(pnms[rownames(pbstat)], names(hlv))
    plotCor(pbstat$median, log(hlv[idx]),
...
plotdev(file.path(fig.path,paste0("protein_iupred3_raas_all")),
        type="png", res=300, width=3.5,height=3.5)
...
plotdev(file.path(fig.path,paste0("volcano_protein_windows_all")),
        type="png", res=300, width=4.4,height=4)
par(mai=c(.5,.5,.1,.5), mgp=c(1.2,.3,0), tcl=-.25)
volcano(winstat, cut=50, v.txt=c(-2,0), p.txt=10,
        ids=ids, xlab=expression(log[10]*bar(RAAS)[all]))
dev.off()
...
plotdev(file.path(fig.path,paste0("volcano_protein_windows_unique")),
        type="png", res=300, width=4.4,height=4)
par(mai=c(.5,.5,.1,.5), mgp=c(1.2,.3,0), tcl=-.25)
volcano(winstat, value="median", cut=20, v.txt=c(-2,-2),
        p.txt=6, ids=unlist(ids), xlab="median of site median RAAS")
...
res <- volcano(hustat, value="median",
               ...
               xlab=expression(log[10](bar(RAAS[unique]))))
dev.off()
