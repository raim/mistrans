...
## NOTE: r=.58 when including + protein.halflife  + protein.meltingT` but
## only for about 500 proteins
model <- randomForest(RAAS.median ~  MMSeq2 + iupred3 +  flDPnn +
                          anchor2 + DisoRDPbind +
                          fromto + codon +
                          protein.length + protein.intensity +
                          protein.halflife  + protein.meltingT,
...
plotdev(file.path(mdlfig.path,"randomForest_noExperimentalData"), type=ftyp,
        width=corW, height=corH, res=300)
par(mai=pmai, mgp=pmpg, tcl=-.25) 
plotCor(predictions, test$RAAS.median, xlab="prediction", ylab=xl.raas,
        title=TRUE, cor.legend=FALSE)
